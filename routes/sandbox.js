...
// Middleware Universal Catch-All (Orice metoda, orice cale)
router.use((req, res) => {
	// Extract the requested mode (default: test)
	const mode = (req.query.omd_mode || 'test').toLowerCase();
	...
	if (mode === 'echo') {
		return res
			.status(200)
			.json({ method: req.method, original_url: req.originalUrl, path: req.path, headers: req.headers, query: req.query, body: req.body || {} });
	}
...
// 5. Payload conversion (XML <-> JSON or JSON -> SOAP)
